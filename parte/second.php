...
$ws=$_GET['ws'];
$f=1;
$result1= "SELECT  wine_name,variety,year,winery_name,region_name,cost,on_hand,SUM(qty),qty*price,on_hand FROM wine 
...
";
  if(lyr>=uyr)
     ...
     {
    $p=0; 
    foreach($db->query($result1) as $row)
   {
   ...
   $_SESSION['winename']  = array_unique(array_merge($_SESSION['winename'], $ary));
   if($_SESSION['procid'])
     {
      $_SESSION['flag']=1;
     $tmp[$p][0]="<a href='sclosing.php?pt=13'>end session</a>";
     $p++;
     $tmp[$p][0]="<a href='first.php?procid=1234'>home</a>";
     }
   else
     {
     $tmp[$p][0]="<a href='first.php'>home</a>";
     }
